...
export default function AdminDashboard() {
  ...
  // Derived KPIs
  const totalPortfolio = portfolioSummary.reduce((sum, i) => sum + (i.expected_amount || 0), 0)
  const totalPayouts = portfolioSummary.reduce((sum, i) => sum + (i.total_payout || 0), 0)
  ...
  const handleAddSource = async () => {
    ...
    const { error } = await supabase
      .from('income_sources')
      .insert([{ source_name: newSource.source_name, expected_amount: Number(newSource.expected_amount) }])
  ...
  return (
    <main className="min-h-screen bg-[#f8f9fa] text-[#0A1E2D] px-6 py-10 font-[Lato]">
      <div className="max-w-7xl mx-auto bg-white p-10 rounded-2xl shadow-md border border-gray-100">
        ...
        <h1 className="text-3xl font-semibold mb-2 text-[#0A1E2D]">Admin Dashboard</h1>
        <p className="text-gray-600 mb-8 text-[15px]">Monitor users, payouts, and portfolio performance in real time.</p>

        {/* KPIs */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-[#fdfbf7] border border-gray-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="bg-[#fdfbf7] border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            <p className="text-3xl font-semibold text-[#C6A664]">{formatCurrency(totalPayouts)}</p>
          </div>
          <div className="bg-[#fdfbf7] border border-gray-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="space-y-10">
                  ...
                  {portfolioSummary.map((item, idx) => (
                    <div key={idx} className="bg-[#fdfbf7] p-5 rounded-xl border border-gray-200 text-center">
                      ...
                      <div className="mt-2 flex justify-center gap-3">
                        ...
                        <button onClick={() => handleDelete(item.id, item.source_name)} className="text-sm text-red-600 hover:underline">Delete</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section>
              ...
              {monthlyTrends.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={350}>
                  <LineChart data={monthlyTrends.map((d) => ({
                    ...d,
        ...
        {showEditModal && editItem && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg w-[400px] shadow-lg">
              ...
              <input type="text" value={editItem.source_name} onChange={(e) => setEditItem({ ...editItem, source_name: e.target.value })} className="w-full border rounded-md p-2 mb-4" />
              <label className="block mb-2 text-sm text-gray-600">Expected Amount</label>
              <input type="number" value={editItem.expected_amount} onChange={(e) => setEditItem({ ...editItem, expected_amount: e.target.value })} className="w-full border rounded-md p-2 mb-6" />
              <div className="flex justify-end gap-3">
                <button onClick={() => setShowEditModal(false)} className="px-4 py-2 rounded-md bg-gray-200 hover:bg-gray-300">Cancel</button>
                <button onClick={handleSaveEdit} className="px-4 py-2 rounded-md bg-[#C6A664] text-[#0A1E2D] hover:bg-[#b59655]">Save Changes</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg w-[400px] shadow-lg">
              ...
              <label className="block mb-2 text-sm text-gray-600">Source Name</label>
              <input type="text" value={newSource.source_name} onChange={(e) => setNewSource({ ...newSource, source_name: e.target.value })} className="w-full border rounded-md p-2 mb-4" />
              <label className="block mb-2 text-sm text-gray-600">Expected Amount</label>
              <input type="number" value={newSource.expected_amount} onChange={(e) => setNewSource({ ...newSource, expected_amount: e.target.value })} className="w-full border rounded-md p-2 mb-6" />
              <div className="flex justify-end gap-3">
                <button onClick={() => setShowAddModal(false)} className="px-4 py-2 rounded-md bg-gray-200 hover:bg-gray-300">Cancel</button>
                <button onClick={handleAddSource} className="px-4 py-2 rounded-md bg-[#C6A664] text-[#0A1E2D] hover:bg-[#b59655]">Add Source</button>
              </div>
            </div>
          </div>
        )}
