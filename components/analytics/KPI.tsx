...
export default function KPI({ insights }: KPIProps) {
  ...
  const avgPayment = insights.reduce((sum, i) => sum + i.avg_payment, 0) / insights.length
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
      {items.map((item, idx) => (
        <div key={idx} className="border rounded-xl shadow-sm p-6 bg-white hover:shadow-md">
          <p className="text-sm text-gray-500">{item.label}</p>
          <p className="text-2xl font-semibold text-[#0A1E2D] mt-1">{item.value}</p>
        </div>
      ))}
    </div>
  )
